...
def historial_paciente(paciente_id: int, db: Session = Depends(get_db), empresa_id: int = Depends(resolve_empresa_id), medico_restriccion: tuple = Depends(get_medico_restriction)):
    ...
    medico_id_auth, es_admin, rol = medico_restriccion
    # FIX: Los médicos usan rol='operador' con medico_id en el JWT, no rol='medico'
...
def turnos_calendario(
    ...
    turnos = []
    if resultados:
        # Cache pacientes en batch para evitar N+1
        new_pac_ids = set()
        old_pac_ids = set()
        for v in resultados:
...
def listar_turnos(
    ...
    # Enriquecer con nombres — batch para evitar N+1
    result = []
    if visitas:
        pac_ids = set()
        med_ids = set()
