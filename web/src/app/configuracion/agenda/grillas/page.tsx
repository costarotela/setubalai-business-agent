...
interface GrillaMedica {
  ...
  medico?: { id: number; nombre: string; apellido: string; especialidades?: string[] };
}
...
export default function GrillasPage() {
  ...
  const [form, setForm] = useState({ medico_id: "", dia_semana: "1", hora_inicio: "08:00", hora_fin: "12:00", activo: true });
  ...
  const openNew = () => {
    setEditId(null);
    setForm({ medico_id: "", dia_semana: "1", hora_inicio: "08:00", hora_fin: "12:00", activo: true });
    setError("");
    setShowModal(true);
  };
  ...
  const openEdit = (g: GrillaMedica) => {
    setEditId(g.id);
    setForm({ medico_id: String(g.medico_id), dia_semana: String(g.dia_semana), hora_inicio: g.hora_inicio, hora_fin: g.hora_fin, activo: g.activo });
    setError("");
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    setError("");
    if (!form.medico_id) { setError("Seleccioná un médico"); return; }
    if (form.hora_inicio >= form.hora_fin) { setError("Hora fin debe ser mayor que hora inicio"); return; }
    const payload = { medico_id: Number(form.medico_id), dia_semana: Number(form.dia_semana), hora_inicio: form.hora_inicio, hora_fin: form.hora_fin, activo: form.activo };
    try {
      let res;
      if (editId) {
        res = await authFetch(`/configuracion-agenda/grillas-medicas/${editId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      } else {
        res = await authFetch("/configuracion-agenda/grillas-medicas/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      }
      if (!res.ok) {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: "center", color: "#62666d" }}>Cargando grillas...</div>;
  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32, paddingBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 600, color: "#f7f8f8", marginBottom: 8 }}>📅 Grillas Horarias</h2>
          <p style={{ fontSize: 13, color: "#62666d", margin: 0 }}>{grillas.length} horarios configurados</p>
        </div>
        <button onClick={openNew} style={{ background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 8, padding: "8px 16px", color: "#3b82f6", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+ Nueva Grilla</button>
      </div>
      ...
      {grillas.length === 0 ? (
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "40px 24px", textAlign: "center" }}>
          <p style={{ color: "#62666d" }}>No hay grillas configuradas. Creá la primera.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={g.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    <td style={{ padding: "14px 20px" }}>
                      <div style={{ fontWeight: 500, color: "#f7f8f8" }}>Dr. {medico?.apellido || g.medico_apellido || "?"}, {medico?.nombre || g.medico_nombre || "?"}</div>
                    </td>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }} onClick={() => setShowModal(false)}>
          <div style={{ background: "#18181b", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 32, width: 480, maxWidth: "90vw" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ fontSize: 20, fontWeight: 600, color: "#f7f8f8", marginBottom: 24 }}>{editId ? "Editar Grilla" : "Nueva Grilla Horaria"}</h3>
